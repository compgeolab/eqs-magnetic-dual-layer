...
\begin{algorithm}[!h]
    ...
    \For{each fold $k$}{
        ...
        \;
        Calculate the Root Mean Square Error (RMSE) between the observed data from the testing set (\textcolor{orange}{$\mathbf{d^{o_t}}$}) and the predicted total-field anomaly (\textcolor{orange}{$\mathbf{d^{t}}$}) using Equation~\ref{eq:rmse}
        \;
    }
