...
The total field anomaly $\Delta T(x, y, z)$, observed at a point $(x, y, z)$ on a Cartesian right-handed coordinate system with $x$ pointing eastward and $z$ pointing upward, is the difference between the measured norm of the total magnetic field $\vec{\mathbf{T}}(x, y, z)$ and the norm of the regional reference field $\vec{\mathbf{F}}(x, y, z)$, usually represented by the International Geomagnetic Reference Field (IGRF), at the time of measurement:

\begin{equation}
    \Delta T(x, y, z) = 
    \norm{\vec{\mathbf{T}}(x, y, z)}
    - \norm{\vec{\mathbf{F}}(x, y, z)}
    \ .
\end{equation}

\noindent
The total magnetic field $\vec{\mathbf{T}}$ is the sum of the anomalous magnetic field vector $\vec{\mathbf{B}}$, which is produced by lithospheric sources, and the regional field vector $\vec{\mathbf{F}}$ \citep{Blakley1995, Langel1998, OliveiraJr2015}. The total field anomaly $\Delta T$ can therefore be written as

\begin{align}
    \Delta T &= 
    \norm{\vec{\mathbf{F}} + \vec{\mathbf{B}}} - \norm{\vec{\mathbf{F}}}\ ,
    \\    
    &= \left(
        \vec{\mathbf{F}} \cdot \vec{\mathbf{F}} + 2\vec{\mathbf{F}} \cdot \vec{\mathbf{B}} + \vec{\mathbf{B}} \cdot \vec{\mathbf{B}}\right)^{\frac{1}{2}} - \vert \vec{\mathbf{F}} \vert 
    \ .
\end{align}

\noindent
For the majority of crustal anomalies measured by airborne and shipborne surveys, $\vert \vec{\mathbf{B}} \vert \ll \vert \vec{\mathbf{F}} \vert$ and $\vert \vec{\mathbf{B}} \vert$ is negligible in the equation above. Consequently, $\Delta T$ can be approximated as \citep{Blakley1995}

\begin{align}
    \Delta T &\approx \left(\vec{\mathbf{F}} \cdot \vec{\mathbf{F}} + 2\vec{\mathbf{F}} \cdot \vec{\mathbf{B}}\right)^{\frac{1}{2}} - \vert \vec{\mathbf{F}} \vert \ ,
    \\
    &= \vert \vec{\mathbf{F}} \vert \left(1 + 2\dfrac{\vec{\mathbf{F}} \cdot \vec{\mathbf{B}}}{\vec{\mathbf{F}} \cdot \vec{\mathbf{F}}}\right)^{\frac{1}{2}} - \vert \vec{\mathbf{F}} \vert \ .
    \label{eq:}
\end{align}
...
\begin{align}
    \Delta T & \approx \vert \vec{\mathbf{F}} \vert \left(1 + \dfrac{\vec{\mathbf{F}} \cdot \vec{\mathbf{B}}}{\vec{\mathbf{F}} \cdot \vec{\mathbf{F}}}\right) - \vert \vec{\mathbf{F}} \vert \ ,
    \\
    &=
    \dfrac{\vec{\mathbf{F}} \cdot \vec{\mathbf{B}}}{\vert \vec{\mathbf{F}} \vert} \ ,
    \\
...
\noindent
in which $\hat{\mathbf{F}}$ is a unit vector in the same direction as the regional field. Additionally, $\vec{\mathbf{F}}$ can be considered constant on the scale of a single survey. Thus, $\Delta T$ is approximately a harmonic function in most crustal magnetic studies \citep{Blakley1995,OliveiraJr2015}.
...
\begin{equation}
\textcolor{orange}{\left\lvert \vec{\mathbf{B}}(x,y,z) \right\rvert} = \left\lvert \sum_{j=1}^{M} \vec{\mathbf{B}}_{j}(x,y,z) \textcolor{teal}{m_j} \right\rvert 
\ .
\end{equation}
