...
\section{Methodology}
The observed total field anomaly vector, $\Delta T$, has N number of data points with positions $(x_i, y_i, z_i)$, where $x_i$ points in the geographic north direction, $y_i$ points in the east direction and $z_i$ points upwards (see black inverted triangles in figure XX). $\Delta T$ is the difference between the measured magnetic total field intensity vector, $\vec{T}$, and the regional reference field vector, $\vec{F}$,
\begin{equation}
    \Delta T = \left\lVert \vec{T} \right\rVert - \left\lVert \vec{F} \right\rVert,
\end{equation}
where the measured $\vec{T}$, is the sum of the anomalous magnetic field vector, $\vec{B}$, and $\vec{F}$ (cite Blakley, 1996, Langel and Hinze, 1998, Oliveira Jr, et al, 2015):
\begin{equation}
    \Delta T = \left\lVert \vec{F} + \vec{B} \right\rVert - \left\lVert \vec{F} \right\rVert.
\end{equation}
Up to a regional-scale, $\hat{F}$ is considered to be constant and the $\vec{B}$ is much smaller in magnitude in comparison to the $\vec{F}$. Subsequently, $\Delta T$ can be approximated as the dot product of the $\vec{B}$ and the regional field direction, $\hat{F}$ (cite Telford, et al, 1990; Blakley, 1996, Oliveira Jr, et al, 2015):
\begin{equation}
